    ...
    <div id="tabs-2">
                ...
                <ul class="tabs__caption">
                    <?php foreach ($stageModels as $key => $stage) { ?>
                      <li class="<?= $key !== 0 ?: 'active' ?>">
                        ...
                        (<?= ($stage->end_date - $stage->start_date) / (60 * 60 * 24) ?> дней)
                      </li>
                    <? } ?>
                </ul>
              </div>
        ...
        <?php if (1 === 2) { ?>
            <? if ($model->priceSections) { ?>
                <? foreach ($model->priceSections as $priceSection) {
                    /* @var $priceSection \common\models\PriceSection */ ?>
              <div class="tour_in tour_know">
                <h2 class="h4"><?= $priceSection->title ?></h2>
                <ul class="price_tour">
                    <? if ($priceSection->priceItems) { ?>
                        <? foreach ($priceSection->priceItems as $priceItem) {
                            /* @var  $priceItem \common\models\PriceItem */ ?>
                        <li>
                          <p><?= $priceItem->text ?></p>
      ...
      Желаемые даты заезда
      <form id="rooms-date-form">
        <input name="id" type="hidden" value="<?= $model->id ?>">
        <input name="dateFrom" type='text' class="datepicker-here" data-date-format="yyyy-mm-dd" value="<?= date("Y-m-d"); ?>"/>
        <input name="dateTo" type='text' class="datepicker-here" data-date-format="yyyy-mm-dd" value="<?= date("Y-m-d", strtotime("+1 month", time())); ?>"/>
        <button class="showRooms">Показать</button>
      </form>
                  ...
                  <? } else { ?>
                      <? if ($accom->rooms) { ?>
                        ...
                        <div class="apart item" style="position: relative; padding-top: 130.72%;">
                          ...
                          <div class="icons_ap">
                              <? if ($room->attributes0) { ?>
                                  <? foreach ($room->attributes0 as $attr) {
                                      /* @var $attr \common\models\Attribute */ ?>
                                  <div class="ico">
      ...
      <? foreach ($model->timetableDays as $timetableDay1) { ?>
        <div id="tabs-1-<?= $timetableDay1->id ?>" class="program_tour">
            <? if ($timetableDay1->timetableItems) { ?>
                ...
                <p>
                  <span><?= Yii::$app->formatter->asDate($timetableItem->start_time,
                          'php:H:i') ?><?= $timetableItem->end_time ? ' - '.Yii::$app->formatter->asDate($timetableItem->end_time, 'php:H:i') : '' ?></span>
                    <?= $timetableItem->text ?>
                </p>
                <? } ?>
            <? } ?>
